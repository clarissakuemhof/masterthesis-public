...
class PGMORL_EA(MOAgent):
    ...
    def __init__(
        ...
        num_envs: int = 4,
        pop_size: int = 6,
        warmup_iterations: int = 2, #default 80, just to see if evolve_population works
        ...
        if env is None:
            if self.seed is not None:
                print("seed env", self.seed)
                # self.seed + i is correct so each env in the vec env has its own unique seed
    ...
    def __task_weight_selection(self, ref_point: np.ndarray):
        ...
        for i in range(len(self.agents)):
            ...
            best_novelty = -float("inf")  # track novelty score
            ...
            for candidate, last_candidate_eval in zip(population, population_eval):
                ...
                '''# Compute novelty scores for each candidate
                novelty_scores = [
                    self.novelty_search.compute_novelty(predicted_eval)
                ...
                if self.log:
                    wandb.log({
                        "new/offspring": wandb.Histogram(novelty_scores),
                        "global_step": self.global_step
                    })'''
                ...
                # Select the best combination of improvement and novelty
                '''mixture_metrics = [
                    hypervolume(ref_point, current_front + [predicted_eval]) - sparsity(current_front + [predicted_eval])
                    + novelty_score * 0.8  # novelty weight
                    for predicted_eval, novelty_score in zip(predicted_evals, novelty_scores)
                ]'''
                ...
                if max_improv < current_candidate_improv:
                    ...
                    #best_novelty = novelty_scores[current_candidate_weight]

            #selected_tasks.append((tuple(best_eval), tuple(best_candidate[1]), best_novelty))
            selected_tasks.append((tuple(best_eval), tuple(best_candidate[1])))
            #self.novelty_search.add_to_archive(best_predicted_eval)  # Add to archive for future novelty computation
            ...
            #copied_agent.fitness = np.sum(best_eval)
    ...
    def train(
        ...
        # Evolution
        # without the 27*, the loop would only run once
        ...
        while iteration < max_iterations:
            ...
            #self.__task_weight_selection_regret(ref_point=ref_point, update_best = True)
